  ...
  return (
    <>
      {cardData.map((item, index) => (
        <div
          key={index}
          className="xs-sm:py-6 grid min-h-60 w-full place-items-start items-start gap-x-10 gap-y-3 border-t border-mybrown-50 pb-8 pt-10 sm:gap-y-4 md:grid-cols-[1fr_3fr_2fr] xl:grid-cols-[1fr_3fr_1fr] max-md:grid-rows-[2em_1fr_auto]"
        >
          <span className="slide-in-left text-[18px] leading-[133%] max-sm:ml-0.5">
            Step {item.order}
          </span>
          <div className="sm:-mt-[4%] xl:-mt-[3.5%]">
